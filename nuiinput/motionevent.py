...
2. All the :class:`MotionEvent` 's are dispatched from
    :meth:`~nuiinput.dispatcher.MotionEventDispatcher.on_motion`.
3. If a :class:`MotionEvent` has a `pos` profile, we dispatch it through
    :meth:`~nuiinput.dispatcher.MotionEventDispatcher.on_touch_down`,
    :meth:`~nuiinput.dispatcher.MotionEventDispatcher.on_touch_move` and
    :meth:`~nuiinput.dispatcher.MotionEventDispatcher.on_touch_up`.
4. You can also listen to changes of the mouse position by watching
:attr:`~nuiinput.dispatcher.MotionEventDispatcher.on_mouse_pos`.
...
class MotionEvent(MotionEventBase):
    ...
    def grab(self, class_instance, exclusive=False):
        ...
        to receive subsequent :meth:`~nuiinput.dispatcher.Dispatcher.on_touch_move`
        and :meth:`~nuiinput.dispatcher.Dispatcher.on_touch_up`
