...
:class:`~nuiinput.core.window.Window`. Dispatching can be disabled by setting
...
:attr:`~nuiinput.motionevent.MotionEvent.profile` property list.
...
pos              2D position. Also reflected in the
                 :attr:`~nuiinput.motionevent.MotionEvent.x`,
                 :attr:`~nuiinput.motionevent.MotionEvent.y`
                 and :attr:`~nuiinput.motionevent.MotionEvent.pos`
                 properties.
...
from nuiinput.base import EventLoop
...
class MouseMotionEvent(MotionEvent):
    ...
    def update_graphics(self, win, create=False):
        ...
        if de is None and create:
            if Color is None:
                from nuiinput.graphics import Color, Ellipse
